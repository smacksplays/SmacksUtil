...
public abstract class AbstractLargeBackpackMenu extends AbstractContainerMenu {
    ...
    public void clicked(int slot_num1, int slot_num2, ClickType clickType, Player player) {
        if (clickType == ClickType.SWAP){
            ...
            ItemStack itemStack2 = playerInventory.getItem(slot_num2);
            if (itemStack2.isEmpty() && itemStack1.getCount() <= itemStack1.getMaxStackSize()) {
                super.clicked(slot_num1, slot_num2, clickType, player);
            } else if (itemStack2.isEmpty() && itemStack1.getCount() > itemStack1.getMaxStackSize()) {
                ...
                itemStack1.setCount(itemStack1.getCount() + itemStack2.getCount());
                ...
                ItemStack temp = itemStack2.copy();
        ...
        if (clickType == ClickType.PICKUP){
            Slot slot1 = player.containerMenu.slots.get(slot_num1);
            ItemStack itemStack1 = slot1.getItem();
            ItemStack itemStack2 = player.containerMenu.getCarried();
            if (itemStack2.isEmpty() && itemStack1.isStackable() && itemStack1.getCount() > itemStack1.getMaxStackSize()) {
                itemStack1.setCount(itemStack1.getCount() - itemStack1.getMaxStackSize());
                slot1.set(itemStack1);
                ItemStack temp = itemStack1.copy();
                temp.setCount(itemStack1.getMaxStackSize());
